...
import { _fetchMockData, fetchMockData } from '../__mock';
import {
  FilterContext,
  TNamedFilter,
...
import { getColumns } from './CancelledEntries.utils';
...
export const CancelledEntriesTable = () => {
  ...
  const isLastPage = data?.pages?.[0]?.meta?.isLast;

  console.log('*** render isLast ', isLastPage);
  ...
  const fetchMoreOnBottomReached = useCallback(
    ...
    [fetchNextPage, isFetching, isLastPage, totalDBRowCount, totalFetched]
  );

  // Используем useEffect с пустым массивом зависимостей для первоначальной загрузки
  useEffect(() => {
